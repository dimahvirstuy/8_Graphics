...
"""
ARG_COMMANDS = [ 'line', 'scale', 'move', 'rotate', 'save', 'circle', 'bezier', 'hermite', 'box', 'sphere', 'torus', 'push', 'pop', 'display' ]

def parse_file( fname, edges, polygons, transform, screen, color, stack):
    ...
    while c < len(lines):
        ...
        if line == 'sphere':
            ...
            matrix_mult(stack, transfrom)

        elif line == 'move':
            #print 'MOVE\t' + str(args)
            t = make_translate(float(args[0]), float(args[1]), float(args[2]))
            matrix_mult(t, transform)
            matrix_mult(stack, transform)
